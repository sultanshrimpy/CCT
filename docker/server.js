...
// ─── Check if user is an authorised admin ────────────────────────────────────
// A user is an admin if:
//   a) They have privileged: true in the users collection (instance owner), OR
//   b) They appear in the cct_admin_users collection (manually promoted)
async function isAdminUser(userId) {
  if (!db) return false;
  try {
    // Revolt uses ULID strings as _id, not MongoDB ObjectIds
    const user = await db.collection("users").findOne({ _id: userId }, { projection: { privileged: 1 } });
    if (user?.privileged) return true;

    // Check the CCT admin users table
    const adminEntry = await db.collection("cct_admin_users").findOne({ userId: userId.toString() });
    return !!adminEntry;
  } catch (e) {
    console.warn("[CCT Admin] isAdminUser check failed:", e.message);
    return false;
  }
}
...
app.post("/api/admin/login", async (req, res) => {
  ...
  if (sessionToken) {
    ...
    const admin = await isAdminUser(user._id);
    if (!admin) return res.status(403).json({ error: "This account does not have admin access. Ask a server owner to promote you." });

    const token = jwt.sign(
      { username: user.username, userId: user._id, role: "admin", method: "revolt_session" },
      JWT_SECRET,
      { expiresIn: "24h" }
    );
    return res.json({ token, username: user.username, method: "revolt_session" });
  }
  ...
  const token = jwt.sign({ username, role: "admin", method: "password" }, JWT_SECRET, { expiresIn: "24h" });
  res.json({ token, username, method: "password" });
});
...
app.get("/api/admin/stats", requireAdmin, async (req, res) => {
  ...
  try {
    const [users, servers, messages, reports] = await Promise.all([
      ...
      db.collection("safety_reports").countDocuments({ status: { $ne: "Resolved" } }),
    ]);
...
app.get("/api/admin/users", requireAdmin, async (req, res) => {
  ...
  // Annotate with CCT admin status
  const adminIds = new Set((await db.collection("cct_admin_users").find({}).toArray()).map(a => a.userId));
  res.json(users.map(u => ({ ...u, cctAdmin: adminIds.has(u._id.toString()) || !!u.privileged })));
});
...
app.get("/api/admin/reports", requireAdmin, async (req, res) => {
  if (!db) return res.json([]);
  res.json(await db.collection("safety_reports").find({}).sort({ _id: -1 }).limit(50).toArray());
});
app.patch("/api/admin/reports/:id", requireAdmin, async (req, res) => {
  if (!db) return res.status(503).json({ error: "DB not connected" });
  await db.collection("safety_reports").updateOne({ _id: req.params.id }, { $set: { status: req.body.status } });
  res.json({ ok: true });
});
